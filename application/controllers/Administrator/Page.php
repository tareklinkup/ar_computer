...
class Page extends CI_Controller {
    ...
    public function getBranchesDue(){
        $data = json_decode($this->input->raw_input_stream);
        // echo json_encode($data);
        // return 0;
        $clauses = "";
        if((isset($data->BranchId)) && ($data->BranchId != "")) {
            $clauses .= " and transfer_to = '$data->BranchId'";
        }
        $branches = $this->db->query("
                select transfer_from, transfer_to, SUM(total_amount) as due
                    from `tbl_transfermaster` 
                    where transfer_from = ? and status = 'a' 
                    $clauses
    ...
    public function addBranch()  {
        ...
        try{
            ...
            if(!empty($_FILES['image'])) {
                ...
                $imageName = round(microtime(true));
                $config['file_name'] = $imageName;
                $this->load->library('upload', $config);
                $this->upload->do_upload('image');
                $config['image_library'] = 'gd2';
                $config['source_image'] = './uploads/branch/'. $imageName ; 
                $config['new_image'] = './uploads/branch/';
                ...
                $this->db->query("update tbl_brunch set image = ? where brunch_id = ?", [$imageName, $branchId]);
            }
    ...
    public function updateBranch()  {
        ...
        try{
            ...
            if(!empty($_FILES['image'])) {
                ...
                $this->image_lib->resize();
                $imageName = $imageName . $this->upload->data('file_ext');
                $imageUnlink = './uploads/branch/'. $branch->image; 
                if(file_exists($imageUnlink)){
                    unlink($imageUnlink);
                }
                $this->db->query("update tbl_brunch set image = ? where brunch_id = ?", [$imageName, $branch->branchId]);
            }
