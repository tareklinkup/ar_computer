    ...
    public function updateProduct(){
        ...
        try{
            ...
            if($productObj->is_serial == 0) {
                $checkPurchaseSerial = $this->db->query("
                    select 
                        * 
                    from tbl_product_serial_numbers ps 
                    where ps.ps_status = 'a'
                    and ps.ps_prod_id = ?
                ", [$productObj->Product_SlNo]);
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(trd.quantity), 0)
                    ...
                    join tbl_transfermaster tm on tm.transfer_id = trd.transfer_id
                    where trd.product_id = p.Product_SlNo
                    and tm.transfer_to = '$branchId' and tm.status = 'a'
                    " . (isset($data->date) && $data->date != null ? " and tm.transfer_date <= '$data->date'" : "") . "
    ...
    public function getProductLedger(){
        ...
        $result = $this->db->query("
            ...
            select 
                ...
                ifnull((prd.PurchaseReturnDetails_ReturnAmount / prd.PurchaseReturnDetails_ReturnQuantity),0) as rate,
                0 as in_quantity,
            ...
            select
                ...
                ifnull((srd.SaleReturnDetails_ReturnAmount / srd.SaleReturnDetails_ReturnQuantity),0) as rate,
