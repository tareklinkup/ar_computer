...
class Customer extends CI_Controller
{
    ...
    public function getCustomers(){
        ...
        if(isset($data->mobile) && $data->mobile != ''){
            $clauses .= " and Customer_Mobile = '$data->mobile'";
            $type = " and Customer_Type = 'G'";
        }
        else {
            $type = " and Customer_Type != 'G'";
        }
    ...
    public function addCustomer()
    {
        ...
        try{
            ...
            if ($customerObj->is_supplier) {
                ...
                $res = ['success'=>true, 'message'=>'Customer with supplier added successfully', 'customerCode'=>$this->mt->generateCustomerCode()];
            }
    ...
    public function updateCustomer()
    {
        ...
        try{
            ...
            $customerImage = $this->db->query("select image_name from tbl_customer where Customer_SlNo =  '$customerId' ")->row();
            if(!empty($_FILES)) {
                $customerImg = './uploads/customers/'.$customerImage->image_name;
                if(file_exists($customerImg)){
                    unlink($customerImg);
                }
                $config['upload_path'] = './uploads/customers/';
                $config['allowed_types'] = 'gif|jpg|png';
            ...
            if ($customerObj->is_supplier) {
                $supplierObj = json_decode($this->input->post('data'));
                $supplierCodeCount  = $this->db->query("select * from tbl_supplier where Supplier_Mobile = ?", $supplierObj->Customer_Mobile)->num_rows();  
                $temp_code  = $this->db->query("select * from tbl_supplier where Supplier_Mobile = ?", $supplierObj->Customer_Mobile)->row();    
                
                if($supplierCodeCount == 0){
                    $code = $this->mt->generateSupplierCode();
                }
                else{
                    $code = $temp_code->Supplier_Code;
                }             
                $supplier["Supplier_brinchid"] = $this->session->userdata("BRANCHid");
                ...
                if($supplierCodeCount == 0){
                        ...
                        $this->db->query("update tbl_supplier set is_customer = ? where Supplier_Code = ?", [ true, $supplierObj->Customer_Code]);  
                        $this->db->where('Supplier_Mobile', $supplierObj->Customer_Mobile)->update('tbl_supplier', $supplier);                     
                        $res = ['success'=>true, 'message'=>'Customer with supplier added successfully', 'customerCode'=>$this->mt->generateCustomerCode()];
                } 
            }
    ...
    public function getCustomerSupplierLedger() {
        ...
        $payments = $this->db->query("
            ...
            join tbl_customer c on c.Customer_SlNo = s.customer_id
            where pm.status = 'a'
