...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases(){
        ...
        if(isset($data->status) && $data->status != '') {
            $clauses .= " and pm.status = '$data->status'";
        }
    ...
    public function addPurchase()
    {
        ...
        try{
            ...
            $purchase = array(
                ...
                'PurchaseMaster_ReferenceNo' => $data->purchase->referenceNo,
    ...
    public function pendingPurchase()
    {
        ...
        $data['title'] = "Pending Purchase Record";
        $data['content'] = $this->load->view('Administrator/purchase/pending_purchase_record', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function  deletePurchase(){
        ...
        try{
            ...
            if($purchase->status != 'a' &&  $purchase->status != 'p'){
    ...
    public function approvePurchase() {
        ...
        try {
            $this->db->trans_begin();
            $data = json_decode($this->input->raw_input_stream);
            $invoice = $this->db->query("select PurchaseMaster_InvoiceNo from tbl_purchasemaster where PurchaseMaster_SlNo = ?", $data->purchaseId)->row()->PurchaseMaster_InvoiceNo;
            $purchaseDetails = $this->db->query("select * from tbl_purchasedetails where PurchaseMaster_IDNo = ? and Status = 'p'", $data->purchaseId)->result();
            
            // update purchase master
            $this->db->set('status', 'a')->where('PurchaseMaster_SlNo', $data->purchaseId)->update('tbl_purchasemaster');

            // update purchase details
            $this->db->set('Status', 'a')->where('PurchaseMaster_IDNo', $data->purchaseId)->where('Status', 'p')->update('tbl_purchasedetails');
            ...
            foreach($purchaseDetails as $product) {
                ...
                $this->db->query("
                    ...
                    set ps_status = 'a'
                    where ps_purchase_inv = ?
                    and ps_prod_id = ?
                    and ps_status = 'p'
                ", [$invoice, $product->Product_IDNo]);
            }
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->db->trans_commit();
                $res->success = true;
                $res->message = 'Purchase approved successfully';
            }
