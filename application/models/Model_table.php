...
class Model_Table extends CI_Model{
    ...
    public function generateTransferPaymentCode() {
        $paymentCode = "TR00001";
        
        $lastPayment = $this->db->query("select * from tbl_transfer_payment order by TPayment_id desc limit 1");
        if($lastPayment->num_rows() != 0){
            $newPaymentId = $lastPayment->row()->TPayment_id + 1;
            $zeros = array('0', '00', '000', '0000');
            $paymentCode = 'TR' . (strlen($newPaymentId) > count($zeros) ? $newPaymentId : $zeros[count($zeros) - strlen($newPaymentId)] . $newPaymentId);
        }
