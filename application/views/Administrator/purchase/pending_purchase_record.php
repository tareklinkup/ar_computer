						...
						<tr v-for="purchase in purchases">
							...
							<td style="text-align:center;">
                                ...
                                <?php if($this->session->userdata('accountType') == 'm' || $this->session->userdata('accountType') == 'a') {?>
                                    <a href="" title="Approve Purchase" @click.prevent="approvePurchase(purchase.PurchaseMaster_SlNo)"><i class="fa fa-check"></i></a>
                                <?php }?>
	...
	new Vue({
		...
		methods: {
			...
			approvePurchase(purchaseId){
				...
				axios.post('/approve_purchase', {purchaseId: purchaseId})
				.then(res => {
					let r = res.data;
					alert(r.message);
					if(r.success){
