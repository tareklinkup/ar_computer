...
<div class="row" id="customerDueList">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;">
		<div class="form-group">
			...
			<div class="col-sm-2">
				<select class="form-control" v-model="searchType" v-on:change="onChangeSearchType" style="padding:0px;">
					...
					<option value="employee">By Marketing Officer</option>
		...
		<div class="form-group" style="display: none" v-bind:style="{display: searchType == 'employee' ? '' : 'none'}">
			<label class="col-sm-2 control-label no-padding-right">Select Marketing Officer</label>
			<div class="col-sm-2">
				<v-select v-bind:options="employee" v-model="selectedEmployee" label="display_name" placeholder="Select Marketing Officer"></v-select>
			</div>
		</div>
	...
	<div class="col-md-12" style="display: none" v-bind:style="{display: dues.length > 0 ? '' : 'none'}">
		...
		<div class="table-responsive" id="reportTable">
			<table class="table table-bordered">
				...
				<tbody>
				    <tr v-for="(data, ind) in dues" v-if="data.dueAmount > 0">
						<td>{{ ind + 1 }}</td>
	...
	new Vue({
		...
		methods:{
			onChangeSearchType() {
				...
				this.selectedSpecialCustomer = null
				...
				else if(this.searchType == 'employee' && this.employee.length == 0) {
					this.getEmployee();
				}
