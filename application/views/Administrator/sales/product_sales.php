...
<div id="sales" class="row">
	<div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
		<div class="row">
			...
			<div class="form-group">
				<label class="col-sm-1 control-label no-padding-right no-padding-left"> Reference no </label>
				<div class="col-sm-2">
					<input type="text" id="referenceNo" class="form-control" v-model="sales.referenceNo" />
				</div>
			</div>
					...
					<div class="row">
						<div class="col-sm-5">
							...
							<div class="form-group row">
								...
								<div class="col-xs-8">
									<input type="text" id="mobileNo" placeholder="Mobile No" class="form-control" v-model="selectedCustomer.Customer_Mobile" v-bind:disabled="selectedCustomer.Customer_Type == 'G' ? false : true" @change="existCustomer" />
								</div>
							</div>
						...
						<div class="col-sm-5">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group row" style="display: none;" :style="{display: serials.length > 0 ? '' : 'none'}">
				...
				<table class="table table-bordered" style="color:#000;margin-bottom: 5px;">
					...
					<tbody style="display:none;" v-bind:style="{display: cart.length > 0 ? '' : 'none'}">
						<tr v-for="(product, sl) in cart">
							...
							<td>
								<a href="" v-on:click.prevent="editFromCart(sl)"><i class="fa fa-edit"></i></a>
									...
									<tr>
										<td>
											<div class="form-group">
												...
												<div class="col-xs-12">
													<input type="number" id="paid" class="form-control" min="0" v-model="sales.paid" v-on:input="calculateTotal" v-bind:disabled="selectedCustomer.Customer_Type == 'G' ? true : false"/>
												</div>
											</div>
										</td>
									</tr>
...
<script>
	...
	new Vue({
		...
		methods:{
			...
			existCustomer() {
				axios.post('/get_customers', {mobile: this.selectedCustomer.Customer_Mobile})
				.then(res => {
					let customer = res.data[0]
					this.selectedCustomer.Customer_Name = customer.Customer_Name;
			...
			async getCustomerDue() {
				if(this.selectedCustomer.is_supplier == true) {
					axios.post('/get_customer_combine_due',{customerId: this.selectedCustomer.Customer_SlNo})
					.then(res=>{
					...
					await axios.post('/get_customer_due',{customerId: this.selectedCustomer.Customer_SlNo})
					.then(res=>{
						if(res.data.length > 0){
							this.sales.previousDue = res.data[0].dueAmount;
						} else {
			...
			addToCart(){
				...
				if(this.serials.length && this.serial == null  && this.barcodeValue == '') {
				...
				if (checkCart.length > 0) {
					checkCart.map((item) => {
						...
						if (storeObj.length > 0 && product.SerialStore.length > 0) {
							...
							if (checkSameI > -1) {
								...
								console.log(product.SerialStore)
			...
			async editFromCart(ind) {
				// console.log(ind)
				let productText = this.products.filter((product) => {
								if(product.Product_SlNo == this.cart[ind].productId) {
									return product.display_text;
								}
							})
				this.selectedProduct.Product_SlNo = this.cart[ind].productId
				this.selectedProduct.ProductCategory_Name = this.cart[ind].categoryName
				this.selectedProduct.Product_Name = this.cart[ind].name
				this.selectedProduct.Product_SellingPrice = this.cart[ind].salesRate
				this.selectedProduct.vat = this.cart[ind].vat
				this.selectedProduct.quantity = this.cart[ind].quantity
				this.selectedProduct.total = this.cart[ind].total
				this.selectedProduct.Product_Purchase_Rate = this.cart[ind].purchaseRate
				this.selectedProduct.warranty = this.cart[ind].warranty
				this.selectedProduct.display_text = productText[0].display_text
			...
			async getSales(){
				await axios.post('/get_sales', {salesId: this.sales.salesId}).then(res=>{
					...
					this.sales.referenceNo = sales.SaleMaster_ReferenceNo;
